...
void dfs(int st, int ed, vector<vector<int>>& grid, vector<vector<int>>& visited, queue<pair<int,int>>& q){
    int n = grid.size();
    
    if(!valid(st, ed,n,n ) || visited[st][ed] || grid[st][ed]==0) return;
    ...
    while(!q.empty()){
        ...
        while(size--){
            ...
            for(int i = 1;i<=4;i++){
                int ind1 = r + row[i-1];
                int ind2 = c + col[i-1];
                if(valid(ind1, ind2, n , n) && !visited[ind1][ind2]){
                    if(grid[ind1][ind2]==1){
                        return cnt;
                    }
                    visited[ind1][ind2] = 1;
