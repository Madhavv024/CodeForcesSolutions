...
bool canFormSquare(int l1, int b1, int l2, int b2, int l3, int b3) {
    ...
    // Case 1: All in a row
    if (b1 == b2 && b2 == b3 && b1 == S && (l1 + l2 + l3 == S)) return true;

    // Case 2: All in a column
    if (l1 == l2 && l2 == l3 && l1 == S && (b1 + b2 + b3 == S)) return true;

    // Case 3: Two on bottom, one on top
    if (b1 == b2 && (l1 + l2) == S && b1 + b3 == S && l3 == S) return true;

    // Case 4: One on bottom, two on top
    if (b2 == b3 && (l2 + l3) == S && b2 + b1 == S && l1 == S) return true;

    // Case 5: One fills full height, other two stacked beside it
    if (b1 == S && (b2 + b3) == S && (l1 + max(l2, l3)) == S &&
        l2 <= S && l3 <= S) return true;
