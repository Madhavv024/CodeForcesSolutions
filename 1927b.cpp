...
void solve() {
    ...
    vector<int> vis(26), A(n);  // vis is a vector of size 26 to track the frequency of each letter
                                // A is a vector of size n to store the input integers

    for (int i = 0; i < n; i++) {
        cin >> A[i];  // Input n integers into array A
    }

    for (int i = 0; i < n; i++) {  // Iterate over each element in A
        for (int j = 0; j < 26; j++) {  // Iterate over all letters of the alphabet (0 to 25)
            if (vis[j] == A[i]) {  // If the frequency of the j-th letter equals the current integer A[i]
                cout << char('a' + j);  // Print the corresponding letter (char) for j-th position
                vis[j]++;  // Increment the frequency of the j-th letter (vis[j])
                break;  // Stop searching for the letter and break out of the inner loop
            }
        }
    }
    cout << '\n';  // Print a newline after outputting all the characters
