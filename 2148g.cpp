...
unordered_map<int, int> divisorsFreqMap;
...
void addDiv(int x){
    for(int i = 1;i<=sqrt(x);i++){
        if(x%i) continue;
        divisorsFreqMap[i]++;
        if(i != x/i) divisorsFreqMap[x/i]++;
    }
}

int maxCountDiv(int x , int g){
    int maxcnt = 0;
    for (int i = 1; i <=sqrt(x); i++)
    {
        if(x%i) continue;
        if(gcd(i , g) == g && i > g) maxcnt = max(maxcnt, divisorsFreqMap[i]);
        if(gcd(x/i , g) == g && x/i > g) maxcnt = max(maxcnt, divisorsFreqMap[x/i]);
    }
    return maxcnt;
}
...
void solve() {
    ...
    {
        addDiv(arr[i]);
        if(prefG !=gcd(arr[i], prefG) ){ 
            ans[i] = i; //any change in pref gcd will add the length till that in ans array -- obs1
        }
        else{
            ans[i] = max(ans[i-1], maxCountDiv(arr[i], prefG));
        }
        prefG = gcd(arr[i], prefG);
    }
