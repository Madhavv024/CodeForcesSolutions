...
int cntSubset(VI &arr , int ind, int sum){
    if(sum == 0){
        return 1;
    }
    if(ind == 0){
    ...
    if(sum < arr[ind]){
        nottake = cntSubset(arr , ind - 1, sum); 
        return nottake;   
    }
    nottake = cntSubset(arr , ind - 1, sum);
    take = cntSubset(arr , ind - 1 , sum - arr[ind]);
    return take + nottake;
}
...
void solve() {
    ...
    if ((sum + k) % 2 != 0) {
        cout << 0; // No solution possible if (sum + k) is odd
        return;
    }
    int target = (sum + k) / 2;
