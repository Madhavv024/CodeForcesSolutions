...
void countingTheFundamental(){
    // A password consists of 3 letters followed by 2 digits.
    // How many such passwords can be formed if:

    // Letters can repeat,

    // Digits can repeat?

    long long ans = 26 * 26 * 26*10*10;
    cout<<ans<<"\n";
}


void simplePermutation(string s){
    int n = s.size();
    long long ans = 1;
    for(int i = n;i>=1;i--){
        ans *= i;
