...
void solve() {
    ...
    for(int i = 2;i<n;i++){
        if(a){
            if(ans.first<ans.second) ans.first = arr[i];
            else ans.second = arr[i];
            // ans.first = min(ans.first,ans.second );
            // ans.first = arr[i];
        }else{
            if(ans.first<ans.second) ans.second = arr[i];
            else ans.first = arr[i];
            // ans.second = max(ans.first, ans.second);
            // ans.second = arr[i];
        }
        a = !a;
    }
