...
void solve() {
    ...
    for(int i=1;i<=n;i++){
        cin>>arr[i];
        sum+=arr[i];
        if(i%2)sum1+=arr[i]; //even indexes
        else sum2+=arr[i]; //odd indexes
    }
    x = sum / n;  // Calculate the target value each element should have
    if (x * n != sum) {  // Check if the sum is divisible by n
        cout << "NO" << endl;
        return;
    }

    y = n / 2;  // This represents half the number of elements (used later)
    if (y * x != sum2) {  // Check if the sum of elements at even indices is evenly divisible by half the size
        cout << "NO" << endl;
        return;
    }
    cout<<"YES"<<endl;
