...
const int maxlen = 2*1e5;

void solve() {
    int n, y; cin>>n>>y;
    // int freq[maxlen];
    vector<int> arr(n), pref(maxlen+1,0);
    ...
    for (int i = 1; i <= maxlen; i++) pref[i] += pref[i-1]; 
    

    ll ans = -1e18;

    //brute force
    // for(int x=2;x<maxlen;x++){
    //     int totalProfit = 0;
    //     for(auto i:arr){      
    //         int price = (i + x - 1) / x;
    //         totalProfit += price;
    //         if(freq[price] > 0) freq[price]--;
    //         else totalProfit -= y;
    ...
    for(int x = 2;x<=maxlen;x++){
        ll totalCost = 0; int range = (maxlen + x - 1) / x;
        for (int i = 1; i <= range; i++)
        {
            int l = (i - 1) * x + 1, r = min(i*x,maxlen);
            int count  = pref[r] - pref[l-1];
            totalCost += (ll)i * count;
            int newTags = max( count - (pref[i] - pref[i-1]) , 0);
            totalCost -= (ll)y * newTags;
        }

        ans = max(totalCost, ans);
    }
