...
int main(){
 ...
 while(n--){
    ...
    ll a1 = dx, b1 = s, c1 = (-x % s + s) % s;
    ll a2 = dy, b2 = s, c2 = (-y % s + s) % s;
    ll x1,y1,g1 = exgcd(a1,b1,x1,y1);
    if(c1%g1) continue;
    ll m1 = b1/g1;
    ll t1 = ( (__int128)x1*(c1/g1) % m1 + m1) % m1;
    ll A = b1, B = b2, C = (t1 - c2% B + B)%B;
    ll u,v,g2 = exgcd(A,B,u,v);
    if(C%g2) continue;
    ans++;
    }
    cout<<ans<<"\n";
