...
/*
   Small Observations
        ...
        to check if layer is complete, the total pieces assembles must in the form 
        of 1 sqare, 3 sq, 5sq, 7sq 

        i need to check that if there exist any perfect square 
        
        ON THAT DAY -->> important

        in range from 2 -> sum and greater than 
        
        prev perfect sum found, if so, cnt++
        
        increase the sum simultaneously
...
void solve() {
    ...
    int cnt = 0, sum = 0, prev = 1;

    for(int i = 0;i<n;i++){
        sum += arr[i];  
        // cout<<"Sum --> " <<sum<<"\n";
        int x = sqrt(sum);
        if(isperfect(sum) && x&1) cnt++;
    }
    cout<<cnt<<"\n";
