...
bool isSubset(vector<int> &arr , int ind , int target ,int n, vector<vector<int>> dp){
    if(target==0) return true;
    if(ind == n) return arr[ind] == target;

    if(dp[ind][target] != -1) return dp[ind][target];
    if(arr[ind] > target) return dp[ind][target] = isSubset(arr , ind + 1 , target, n , dp);

    return dp[ind][target] = isSubset(arr , ind +1 , target - arr[ind] , n , dp) || isSubset(arr , ind + 1 , target, n , dp);    
