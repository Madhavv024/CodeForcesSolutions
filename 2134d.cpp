...
void dfs(int node, int nodeParent){
    parent[node] = nodeParent;
    for(int ngh:adj[node]){
        if(ngh != nodeParent){
            level[ngh] = level[node] + 1;
            dfs(ngh, node);
        }
    }
}
...
void solve() {
    ...
    level.assign(n+1, 0); // assign n+1 value with default 0 value;
    parent.assign(n+1, -1); // assign n+1 value in parent with default -1 value;
    dfs(1 , -1);
    
    int node1 = max_element(level.begin() , level.end()) - level.begin(); // will subtract last iterator in order to get highest length of tree
    level.assign(n+1, 0); // assign n+1 value with default 0 value;
    parent.assign(n+1, -1); // assign n+1 value in parent with default -1 value;
    dfs(node1 , -1); //node1 is starting point of tree with maxlen

    int node2 = max_element(level.begin() , level.end()) - level.begin(); 

    int prev = -1;

    while(node2!=-1 && adj[node2].size()<=2){
        prev = node2;
        node2 = parent[prev];
    }
