...
void solve() {
    ...
    for(auto &arri:matrix){
        int len; cin>>len;
        arri.resize(len);
        for(auto &x:arri) cin>>x; 
    }
    
    //sort on the basis of length of different arrays
    
    sort(matrix.begin(), matrix.end(), [](vector<int> a , vector<int> b){
        return a.size()<b.size();
    });
    
    int anslen = matrix.back().size();
    vector<int> ans(anslen , 1e9);

    vector<int> edit; edit.pb(0);
    ...
    for(auto &arr:matrix){
        ...
        for (int i = 0; i < (int)edit.size(); i++)
        {
            ...
            for(int i = l;i<r;i++){
                ...
                if(ans[i]>arr[i]) {replace = 1; break;}
            }
